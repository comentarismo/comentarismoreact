...
// Copyright Thomas Modeneis 2018
//
...
export default class ReactBubbleChartD3 {
    constructor (el, props = {}) {
        this.legendSpacing = typeof props.legendSpacing === 'number'
            ? props.legendSpacing
            : 3
    ...
    adjustSize (el) {
        ...
        this.bubble = d3.pack()
        .size([this.diameter, this.diameter]).padding(3)
    }
    ...
    configureLegend (el, props) {
        ...
        const legendRectSize = Math.min(((el.offsetHeight - 20) -
            (this.colorLegend.length - 1) * this.legendSpacing) /
            this.colorLegend.length, 18)
        const legendHeight = this.colorLegend.length *
            (legendRectSize + this.legendSpacing) - this.legendSpacing
        ...
        legendKeys.append('rect').
            attr('width', Math.abs(legendRectSize) * 1).
            attr('height', Math.abs(legendRectSize) * 1).
            style('fill', c => c.color).
            style('stroke', c => c.color)
        
        // add necessary labels to the legend
        legendKeys.append('text').
            attr('x', Math.abs(legendRectSize) * 1 + 2).
            attr('y', Math.abs(legendRectSize) * 1 - 4).
            text(c => c.text)
    }
    ...
    update (el, props) {
        ...
        const color = d3interpolate.interpolateRgb(
            props.fixedDomain ? props.fixedDomain.min : d3.min(data,
                d => d.colorValue),
            props.fixedDomain ? props.fixedDomain.max : d3.max(data,
                d => d.colorValue))
        // .gamma(this.colorRange)
        
        // define a color scale for text town
        const textColor = d3interpolate.interpolateRgb(
            props.fixedDomain ? props.fixedDomain.min : d3.min(data,
                d => d.colorValue),
            props.fixedDomain ? props.fixedDomain.max : d3.max(data,
                d => d.colorValue))
        // .gamma(this.textColorRange)
        
        var root = d3.hierarchy(data.length ? {children: data} : data)
        .sum(function(d) { return d.value; })
        .sort(function(a, b) { return b.value - a.value; });
            // filter(d => d.depth) // filter out the outer bubble
        
        var nodes = this.bubble(root)
        ...
        circles.transition().
            ...
            style('opacity', 1).
            style('fill',
                d => d.selected ? this.selectedColor : color(d.colorValue))
        // for the labels we transition their height, width, left, top, and color
        labels.on('mouseover', this._tooltipMouseOver.bind(this, color, el)).
            transition().
            duration(duration).
            delay((d, i) => i * delay).
            ...
            style('opacity', 1).
            style('color', d => d.selected ? this.selectedTextColor : textColor(
                d.colorValue)).
            attr('class', d => {
                var size
            ...
            .style('font-size',
                d => fontFactor ? fontFactor * d.r + 'px' : null)
        ...
        if (nodes.children.length) {
            ...
            labels.enter().
                ...
                style('top', d => d.y - d.r + 'px').
                style('color',
                    d => d.selected ? this.selectedTextColor : textColor(
                        d.colorValue)).
                style('opacity', 0).
                transition().
                duration(duration * 1.2).
        ...
        circles.exit().transition().duration(duration).attr('transform', d => {
        ...
        labels.exit().transition().duration(duration).style('top', d => {
            const dy = d.y - this.diameter / 2
            const dx = d.x - this.diameter / 2
            const theta = Math.atan2(dy, dx)
            const destY = this.diameter * (1 + Math.sin(theta) ) / 2
    ...
    _tooltipMouseOver (color, el, d, i) {
        ...
        for (var {css, prop, display} of this.tooltipProps) {
            console.log("_tooltipMouseOver, ",css,prop,display)
            this.tooltip.select('.' + css).
                html((display ? display + ': - karaidiasa -' : '') + d[prop])
        }
