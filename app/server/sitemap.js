...
import  {
    getAllPluckDistinct,
    getAllByIndexPluckDistinct,
} from './comentarismo_api'
...
function generateSitemap (conn, cb) {
    ...
    getAllPluckDistinct(conn, 'news', 'operator', function (err, operators) {
        if (err || !operators) {
            logger.error('Error: generateSitemap, getAllPluckDistinct', err)
        }
        
        sitemap.add({
            url: '/commentators_product/operator/amazon/index.xml',
            changefreq: 'daily',
        })
        
        for (var i = 0; i < operators.length; i++) {
            sitemap.add(
                {url: '/news/operator/' + operators[i], changefreq: 'daily'})
            sitemap.add({
                url: '/commentators/operator/' + operators[i],
                changefreq: 'daily',
            })
            sitemap.add({
                url: '/news/operator/' + operators[i] + '/index.xml',
                changefreq: 'daily',
            })
            sitemap.add({
                url: '/commentators/operator/' + operators[i] + '/index.xml',
                changefreq: 'daily',
            })
        }
        ...
        getAllPluckDistinct(conn, 'news', 'languages', function (err, langs) {
            if (err || !operators) {
                logger.error('Error: generateSitemap, getAllPluckDistinct',
                    err)
            }
            for (var i = 0; i < langs.length; i++) {
                sitemap.add(
                    {url: '/news/languages/' + langs[i], changefreq: 'daily'})
                sitemap.add({
                    url: '/commentators/languages/' + langs[i] + '/index.xml',
                    changefreq: 'daily',
                })
                sitemap.add({
                    url: '/news/languages/' + langs[i] + '/index.xml',
                    changefreq: 'daily',
                })
                sitemap.add({
                    url: '/commentators/languages/' + langs[i] + '/index.xml',
            ...
            getAllPluckDistinct(conn, 'news', 'genre', function (err, genres) {
                ...
                for (var i = 0; i < genres.length; i++) {
                    sitemap.add(
                        {url: '/news/genre/' + genres[i], changefreq: 'daily'})
                    //sitemap.add({url: '/commentators/genre/' + genres[i].genre});
                    sitemap.add({
                        url: '/news/genre/' + genres[i] + '/index.xml',
                        changefreq: 'daily',
                    })
                    //sitemap.add({url: '/commentators/genre/' + genres[i].genre + '/index.xml'});
                }
                
                getAllPluckDistinct(conn, 'news', 'countries',
                    function (err, countries) {
                        if (err || !operators) {
                            logger.error(
                                'Error: generateSitemap, getAllPluckDistinct',
                                err)
                        }
                        for (var i = 0; i < countries.length; i++) {
                            sitemap.add({
                                url: '/news/countries/' + countries[i],
                                changefreq: 'daily',
                            })
                            sitemap.add({
                                url: '/commentators/countries/' + countries[i],
                                changefreq: 'daily',
                            })
                            sitemap.add({
                                url: '/news/countries/' + countries[i] +
                                '/index.xml', changefreq: 'daily',
                            })
                            sitemap.add({
                                url: '/commentators/countries/' + countries[i] +
                                '/index.xml',
...
function generateIndexXml (table, index, value, conn, cb) {
    ...
    getAllByIndexPluckDistinct(table, index, value, pluck, conn,
        function (err, values) {
            if (err || !values) {
            ...
            if (table === 'news') {
                ...
                for (var i = 0; i < values.length; i++) {
                    sitemap.add({
                        url: '/sentiment/' + encodeURIComponent(values[i].url),
                        changefreq: 'daily',
                    })
                }
                return cb(null, sitemap.toString())
                ...
                for (var i = 0; i < values.length; i++) {
                    sitemap.add({
                        url: '/product/' +
                        encodeURIComponent(values[i].titleurlize),
                        changefreq: 'daily',
                    })
                }
                return cb(null, sitemap.toString())
                ...
                for (var i = 0; i < values.length; i++) {
                    sitemap.add({
                        url: '/commentators_product/' +
                        encodeURIComponent(values[i].id), changefreq: 'daily',
                    })
                }
                return cb(null, sitemap.toString())
            }
